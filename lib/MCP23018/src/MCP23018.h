...
public:
    ...
    [[nodiscard]] bool digitalRead(uint8_t pin) const;

    static void setBit(uint16_t &val, uint8_t bit, bool state);

    static bool checkBit(uint16_t val, uint8_t bit);

    static void writeRegister(uint8_t reg, uint16_t val);

    static void writeSingleRegister(uint8_t reg, uint8_t val);
...
private:
    ...
    uint16_t _gpioInterruptEnableRegister{};
    uint16_t _gpioPullupRegister{};
    uint16_t _outputLatchRegister{};

    uint16_t _interruptControlRegister{};
    uint16_t _defvalReg{};
